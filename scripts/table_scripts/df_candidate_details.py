...
def candidate_details(bucket):
    cand_id = Append_All(bucket).append_all("Talent")

    # phone number needs the format changing, but in this situation our time can be better spent
    cand_id = cand_id[
        ["name", "id", "gender", "dob", "invited_date", "email", "phone_number", "address", "city", "invited_by", "uni",
         "degree", "month"]]
